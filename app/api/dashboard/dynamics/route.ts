...
import { getDashboardDynamics } from "@/lib/dashboard/queries";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = getStandardDashboardCacheKey("dynamics", filters);
    ...
    const payload = { dynamics: await getDashboardDynamics(filters) };
    if (cacheKey) await writeDashboardApiCache("dynamics", cacheKey, payload);
    return NextResponse.json(payload, { headers: cacheHeaders });
  } catch (error) {
    console.error("dynamics api error", error);
    return NextResponse.json({ message: "Не вдалося отримати динаміку." }, { status: 400 });
  }
}
