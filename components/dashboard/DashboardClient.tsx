...
const chartHelpTexts = {
  institutions:
    "У цьому блоці показано заклади освіти з найбільшою кількістю осіб за вибраними фільтрами. Натисніть «Показати ще», щоб розширити список. Якщо ви виберете конкретні заклади у фільтрах, вони з’являться на початку списку й будуть виділені окремим кольором. Для порівняння динаміки можна додати кілька дат.",
  regions:
    "У цьому блоці показано розподіл кількості осіб за регіонами. Вибрані регіони піднімаються вгору списку та виділяються окремим кольором, щоб їх було зручно порівняти із загальною картиною. Якщо вибрано один або кілька закладів освіти, вони відображаються всередині відповідного регіону з абсолютним значенням і часткою від регіонального показника у відсотках.",
  fields:
    "У цьому блоці показано дані за галузями знань і спеціальностями. Галузі показують розподіл від загального показника, а спеціальності — деталізацію всередині кожної галузі з часткою у відсотках від цієї галузі. Для точнішого перегляду скористайтеся фільтрами за галузями або спеціальностями.",
  dynamics:
