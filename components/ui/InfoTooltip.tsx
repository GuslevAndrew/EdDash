...
export function InfoTooltip({ text }: { text: string }) {
  return (
    <span className="group relative inline-flex align-middle">
      ...
      <span className="pointer-events-none absolute left-1/2 top-7 z-30 hidden w-72 -translate-x-1/2 rounded-md border border-line bg-white/98 px-3 py-2 text-left text-xs font-medium leading-5 text-slate-700 shadow-dropdown group-hover:block group-focus-within:block">
        {text}
      </span>
    </span>
  );
